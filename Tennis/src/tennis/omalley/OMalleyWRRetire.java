...
public final class OMalleyWRRetire
{
	...
	public OMalleyWRRetire(final double alphaA, final double alphaB, final double decay, final boolean withRetirement)
	{
		this.alphaA = alphaA;
		this.alphaB = alphaB;
		this.decay = decay;
	...
	public OMalleyWRRetire(final double alpha, final double decay, final boolean withRetirement)
	{
	...
	public double gameInProgress(final double pa, final double pb, final RetirementRisk risk, final CurrentGameScore gameScore, final boolean serving)
	{
		...
		{
			return risk.ra + (1 - risk.ra) * (1 - risk.rb) * (p * gameInProgress(pa, pb, risk, gameScore.incTargetPoints(), serving)
								...
								final boolean servingNext)
	{
		...
		final double gWR = gameInProgress(pa, pb, risk, gameScore, servingNext);
		final double g = new OMalleyWRWin(alphaA, alphaB, decay, true).gameInProgress(pa, pb, risk, gameScore, servingNext);
		return gWR + (g * setInProgress(pa, pb, risk, setScore.incTargetGames(), new CurrentGameScore(), !servingNext) + (1 - g) * setInProgress(pa, pb, risk, setScore.incOpponentGames(), new CurrentGameScore(), !servingNext));
	}
								     	 ...
								     	 final boolean servingNext, final int numSetsForWin)
	{
		...
		{
			final double sWR = setInProgress(pa, pb, risk, setScore, gameScore, servingNext);
			final double s = new OMalleyWRWin(alphaA, alphaB, decay, true).setInProgress(pa, pb, risk, setScore, gameScore, servingNext);
