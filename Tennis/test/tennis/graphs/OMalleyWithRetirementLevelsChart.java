...
public class OMalleyWithRetirementLevelsChart extends XYLineChart
{
	...
	public OMalleyWithRetirementLevelsChart(final double p, final double q, final boolean servingNext, final int numSetsToWin, final double retirementRisk) throws IOException
	{
	...
	protected XYDataset createDataset()
	{
		...
		for (int i = 1; i < 3; i++)
		{
			for (int j = 0; j < 7; j++)
			{
				...
				final MatchAnalysis analysis = OMalleyCount.matchInProgressCount(p, q, new CurrentMatchScore(i, i-1),
	...
	{
	    final OMalleyWithRetirementLevelsChart chart = new OMalleyWithRetirementLevelsChart(0.55, 0.46, true, 3, 0.2);
	    chart.buildChart();
	    chart.pack();
